...
class LegacyDynamicEnumFieldDefinitionMapper implements FieldDefinitionMapperInterface
{
    /**
     * @inheritDoc
     */
    ...
    {
        return 'legacy-dynamicenum-fields';
    }
    ...
    public function map(FieldDefinition $definition): void
    {
        ...
        foreach ($vardefs as $fieldName => $fieldDefinition) {
            ...
            $type = strtolower($fieldDefinition['type'] ?? '');

            if($type === 'dynamicenum'){

                $fieldDefinition['type'] = 'enum';
                $fieldDefinition['dynamic'] = true;
