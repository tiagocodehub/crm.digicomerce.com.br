...
class EmailManSendDateTimeMapper implements FieldMapperInterface
{
    public const FIELD_NAME = 'send_date_time';
    ...
    /**
     * EmailManSendDateTimeMapper constructor.
     */
    ...
    public function toApi(SugarBean $bean, array &$container, string $alternativeName = ''): void
    {
        ...
        if (empty($bean->$name) && !empty($bean->$alt)) {
            $dbDate = $this->dateFormatService->toDBDateTime($bean->$alt);
            $container[$alt] = $dbDate;
            return;
        }

        $dbDate = $this->dateFormatService->toDBDateTime($bean->$name);

        $container[$name] = $dbDate;
    }
