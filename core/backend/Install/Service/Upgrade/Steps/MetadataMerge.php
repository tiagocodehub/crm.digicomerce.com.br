...
use App\Install\LegacyHandler\Upgrade\MetadataMergeHandler;
 ...
 */
class MetadataMerge implements UpgradeStepInterface
{
    use ProcessStepTrait;

    public const HANDLER_KEY = 'metadata-merge';
    public const POSITION = 850;
    public const STAGE = 'upgrade-finalize';

    /**
     * @var MetadataMergeHandler
     */
    private $handler;

    /**
     * MetadataMerge constructor.
     * @param MetadataMergeHandler $handler
     */
    public function __construct(MetadataMergeHandler $handler)
    {
    ...
    public function execute(array &$context): Feedback
    {
        ...
        $mergeStrategy = $context['metadata-merge'] ?? '';

        return $this->handler->run($targetVersion, $mergeStrategy);
    }
}
