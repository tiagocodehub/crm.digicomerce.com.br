...
class TrackerLinksParser implements CampaignEmailParserInterface
{
    ...
    public function parse(Record $record, array $context): void
    {
        ...
        if ($trackerId === null) {
            $this->logger->debug(
                'Campaigns:TrackerLinksParser::parse - missing trackerId - id: ' . $trackerId, [
                    'trackerId' => $trackerId,
                ]
            );
            return;
        ...
        $attributes['description'] = $this->trackerManager->addTrackerLinks($trackerId, $attributes['description'] ?? '', $context);
        $attributes['description_html'] = $this->trackerManager->addTrackerLinks($trackerId, $attributes['description_html'] ?? '', $context);

        $record->setAttributes($attributes);

        $this->logger->debug(
            'Campaigns:TrackerLinksParser::parse - added unsubscribe link - id: ' . $trackerId, [
                'trackerId' => $trackerId,
